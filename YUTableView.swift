...
public protocol YUTableViewDelegate {
    /**  Called inside "cellForRowAtIndexPath:" method. Edit your cell in this function. */
...
open class YUTableView: UITableView
{
    ...
    open var insertRowAnimation: UITableViewRowAnimation = .middle
    open var deleteRowAnimation: UITableViewRowAnimation = .middle
    open var animationCompetitionHandler: () -> Void = {}
    /** Removes other open items before opening a new one */
    open var allowOnlyOneActiveNodeInSameLevel: Bool = false
    ...
    fileprivate func initializeDefaultValues () {
        self.delegate = self
        self.dataSource = self
    }
    
    open func setDelegate (_ delegate: YUTableViewDelegate) {
        yuTableViewDelegate = delegate
    }
    
    open func setNodes (_ nodes: [YUTableViewNode]) {
    ...
    open func selectNodeAtIndex (_ index: Int) {
    ...
    open func selectNode (_ node: YUTableViewNode) {
...
extension YUTableView: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if nodesToDisplay != nil {
            let dix = ["rowCount": nodesToDisplay.count]
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: "MainViewFrame"), object: nil, userInfo: dix)
...
extension YUTableView: UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        else if (node.hasChildren() && node.cellIdentifier=="BasicCell"){//else if  {//
...
private extension YUTableView {
    ...
    func openNodeAtIndexRow (_ indexRow: Int) {
        ...
        if let newNodes = node.childNodes {
            nodesToDisplay.insert(contentsOf: newNodes, at: indexRow + 1)
